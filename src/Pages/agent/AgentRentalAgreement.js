                  ...
                  <Descriptions size="small">
                    ...
                    <Descriptions.Item label="Address" span={3}>
                      {postInfo.propertyName +
                        ", " +
                        postInfo.propertyAddress +
                        ", " +
                        postInfo.propertyCity +
                        " " +
                        postInfo.propertyPostcode}
                    </Descriptions.Item>
      ...
      if (addRentalError) {
        ...
        if (agentError) {
          ...
          let { data, error } = await supabase
            .from("agent")
            .update({
              properties_rented_out: agentData[0].properties_rented_out + 1,
            })
            .eq("agent_id", postInfo.propertyAgentID);
      ...
      <div style={{ display: "flex", flexDirection: "column" }}>
        ...
        >
          <Descriptions bordered title="Property Details" size="medium">
            ...
            <Descriptions.Item label="Furnish Type" span={3}>
              {postInfo?.propertyFurnishType}
            </Descriptions.Item>
            <Descriptions.Item label="Property Category" span={3}>
              {postInfo?.propertyCategory}
            </Descriptions.Item>
            <Descriptions.Item label="Rental Price" span={3}>
              {postInfo?.propertyPrice.toFixed(2)}
            </Descriptions.Item>
            <Descriptions.Item label="Utility Deposit (RM)" span={3}>
              {(postInfo?.propertyPrice * 2.5).toFixed(2)}
            </Descriptions.Item>
            <Descriptions.Item label="Security Deposit (RM)" span={3}>
              {(postInfo?.propertyPrice * 0.1).toFixed(2)}
            </Descriptions.Item>
            <Descriptions.Item label="Property Type" span={3}>
              {postInfo?.propertyType}
            </Descriptions.Item>
            <Descriptions.Item label="Agent" span={3}>
              {agentName}
            </Descriptions.Item>
          </Descriptions>
        </div>
